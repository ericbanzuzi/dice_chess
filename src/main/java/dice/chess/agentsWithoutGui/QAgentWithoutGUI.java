...
public class QAgentWithoutGUI implements Serializable {
    
    private Types.States prevState1;
    private Types.States prevState2;
    private Types.States prevState3;
    private Types.States prevState4;
    private Types.States prevState5;
    private Types.States prevState6;
    private Types.States currState1;
    private Types.States currState2;
    private Types.States currState3;
    private Types.States currState4;
    private Types.States currState5;
    private Types.States currState6;
    ...
    public QAgentWithoutGUI(float gamma, float epsilon, float alpha, int numActions, int numXYStates,
        ...
        if (!trained) {
            this.qTable =
                    new double[numXYStates][numXYStates][numPieceStates][numXYStates][numXYStates][numPieceStates][numActions];
            this.alphaTable =
                    new int[numXYStates][numXYStates][numPieceStates][numXYStates][numXYStates][numPieceStates][numActions];
        } else {
            deserializeQTable7D();
        ...
        for (int i = 0; i < this.numXYStates; i++) {
            for (int j = 0; j < this.numXYStates; j++) {
                for (int k = 0; k < this.numPieceStates; k++) {
                    double[] qTablePart = this.qTable[stateIndex1][stateIndex2][stateIndex3][i][j][k];
                    int maxIndex = getMaxValue(qTablePart, actionsToCheck);
                    maxIndexMap.put(maxIndex, new int[]{ i, j, k });
                }
            }
        }
        
        Map<Integer, Double> maxValueMap = new HashMap();
        for (Map.Entry<Integer, int[]> set : maxIndexMap.entrySet()) {
            maxValueMap.put(set.getKey(),
                    this.qTable[stateIndex1][stateIndex2][stateIndex3][set.getValue()[0]][set.getValue()[1]][set.getValue()[2]][set.getKey()]);
        }
        
        int maxIndex =
                Collections.max(maxValueMap.entrySet(), Comparator.comparingDouble(Map.Entry::getValue)).getKey();
        
        //update epsilon value
        this.epsilon = (float) (1.0 / (1 + Math.log(1 + Arrays.stream(
                this.alphaTable[stateIndex1][stateIndex2][stateIndex3][maxIndexMap.get(maxIndex)[0]][maxIndexMap.get(
                        maxIndex)[1]][maxIndexMap.get(maxIndex)[2]]).sum())));
    ...
    protected void updateQTable(int state1, int state2, Piece piece, int state4, int state5, Piece enemyPiece,
            int currentAction, float reward, MoveState moves) {
        
        Types.States[] allStates = Types.States.class.getEnumConstants();
        if (this.prevState1 == null && this.prevState2 == null && this.prevState4 == null && this.prevState5 == null) {
            for (Types.States states : allStates) {
                ...
                if (states.name().equalsIgnoreCase("x" + state4)) {
                    this.prevState4 = states;
                }
                if (states.name().equalsIgnoreCase("y" + state5)) {
                    this.prevState5 = states;
                }
                ...
                } else if (enemyPiece.getPlayer().getPlayerColor().equalsIgnoreCase("black") && states.name()
                        .equalsIgnoreCase("black" + enemyPiece.getPieceType().name())) {
                    this.prevState6 = states;
                }
            }
        }
        ...
        for (Types.States cStates : allStates) {
            ...
            } else if (enemyPiece.getPlayer().getPlayerColor().equalsIgnoreCase("black") && cStates.name()
                    .equalsIgnoreCase("black" + enemyPiece.getPieceType().name())) {
                this.currState6 = cStates;
            }
            if (state4 == cStates.value) {
                this.currState4 = cStates;
            }
            if (state5 == cStates.value) {
                this.currState5 = cStates;
            }
        }
        
        //Adaptive alpha
        alphaTable[this.currState1.value][this.currState2.value][this.currState3.value][this.currState4.value][this.currState5.value][this.currState6.value][currentAction]++;
        alpha = (float) (1.0 / Math.log(1
                + alphaTable[this.currState1.value][this.currState2.value][this.currState3.value][this.currState4.value][this.currState5.value][this.currState6.value][currentAction]));
        ...
        double update =
                reward + gamma * findMaxQState(this.currState1.value, this.currState2.value, this.currState3.value,
                        moves)
                        - this.qTable[this.prevState1.value][this.prevState2.value][this.prevState3.value][this.prevState4.value][this.prevState5.value][this.prevState6.value][this.prevAction];
        this.qTable[this.prevState1.value][this.prevState2.value][this.prevState3.value][this.prevState4.value][this.prevState5.value][this.prevState6.value][this.prevAction] =
                this.qTable[this.prevState1.value][this.prevState2.value][this.prevState3.value][this.prevState4.value][this.prevState5.value][this.prevState6.value][this.prevAction]
                        + alpha * update;
        ...
        this.prevState4 = this.currState4;
        this.prevState5 = this.currState5;
        this.prevState6 = this.currState6;
    }
    ...
    private double findMaxQState(int stateIndex1, int stateIndex2, int stateIndex3, MoveState moves) {
        ...
        double maxValue =
                Collections.max(maxValueMap.entrySet(), Comparator.comparingDouble(Map.Entry::getValue)).getValue();
    ...
    /**
     * Writing the 7-D Q-Table to a txt file locally
     */
    protected void writeTableToFile7D() {
        
        StringBuilder builder = new StringBuilder();
        for (int i = 0; i < this.numXYStates; i++) {
            for (int j = 0; j < this.numXYStates; j++) {
                for (int k = 0; k < this.numPieceStates; k++) {
                    for (int l = 0; l < this.numXYStates; l++) {
                        for (int m = 0; m < this.numXYStates; m++) {
                            for (int n = 0; n < this.numPieceStates; n++) {
                                for (int o = 0; o < this.numActions; o++) {
                                    builder.append(this.qTable[i][j][k][l][m][n][o] + "");
        ...
        try {
            BufferedWriter writer = new BufferedWriter(new FileWriter("src/main/resources/QTable7D.txt"));
    ...
    protected void serializeQTable7D() {
        
        try {
            FileOutputStream fileOut = new FileOutputStream("src/main/resources/QTable7D.ser");
    ...
    private void deserializeQTable7D() {
        
        try {
            FileInputStream fileIn = new FileInputStream("src/main/resources/QTable7D.ser");
    ...
    public void play(List<Piece> whitePieces, GameWithoutGUI chessBoardWindow, MoveState moves, Board board) {
    ...
    public void play(Piece whitePiece, GameWithoutGUI chessBoardWindow, MoveState moves, Board board) {
    ...
    protected float getReward(Board board, GameWithoutGUI chessBoardWindow) {
