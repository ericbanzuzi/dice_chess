...
public class MiniMaxAgent {
    ...
    public MiniMaxAgent(int maxDepth, boolean alphaBeta) {
    ...
    // public for dice.chess.tests
    public MoveState alphaBeta(Board board, List<Piece> movablePieces, int depth, boolean maximizing, String maxColor,
...
//    public void playQ(ChessBoardWindow chessBoardWindow, Board board, DrawableDie drawableDie, boolean white,
//            boolean black) {
//
//        Player max;
//
...
//        float alpha = 0.15f;
//        if (qAgent == null) {
//            this.qAgent = new QAgent(gamma, epsilon, alpha, 64, 8, 12, moves);
//        } else {
...
//    }
//
//    public void updateQAgentAfterMove(ChessBoardWindow chessBoardWindow, Board board, MoveState moves) {
//
...
//    }
    ...
    // method to use for GUI
    public void play(ChessBoardWindow chessBoardWindow, Board board, DrawableDie drawableDie, boolean white,
            boolean black) {
        ...
        new Thread(() -> {
            ...
            MoveState move;
            if (alphaBeta) {
                move = alphaBeta(board, movablePieces, maxDepth, true, max.getPlayerColor(), Integer.MIN_VALUE,
                        Integer.MAX_VALUE);
            } else {
                        ...
                        if (maxColor.equalsIgnoreCase("white")) {
                            if (piece.getPlayer().getPlayerColor().equalsIgnoreCase(maxColor)) {
                                score += piece.getValue()
                                        + piece.getStatePositionValuesWhite()[piece.getPosY()][piece.getPosX()];
                            } else {
                                score -= (piece.getValue()
                                        + piece.getStatePositionValuesBlack()[piece.getPosY()][piece.getPosX()]);
                            }
                        } else {
                            if (piece.getPlayer().getPlayerColor().equalsIgnoreCase(maxColor)) {
                                score += piece.getValue()
    ...
    private double evaluate(Board board, int depth, String maxColor) {
        ...
        if (depth == 0) {
            Piece pieceToBeMoved = board.board[Integer.parseInt(board.getLastMove().get(3))][Integer.parseInt(
                    board.getLastMove().get(2))];
