...
public class MiniMaxTest {
    ...
    {
        MiniMaxAgent mm = new MiniMaxAgent(1, true);
        ...
        System.out.println(board.board[4][2]);
        ...
        MoveState move = mm.minimax(board,moveablePieces,1,true,maxColor);
        System.out.println("Move from minmax: " + move.toString());
    ...
    @Test
    public void testRookKing()
    {
        MiniMaxAgent mm = new MiniMaxAgent(1, true);
        ChessPieceFactory cpf = new ChessPieceFactory();
        List<DrawablePieces> drawablePieces = cpf.createDrawablePieces();
        Board board = new Board(drawablePieces);
        //Move knight in position to be able to take white rook or white king
        board.movePiece(board.board[0][1],2,2);
        board.movePiece(board.board[2][2], 4,1);
        board.movePiece(board.board[4][1],6,2);
        board.movePiece(board.board[6][3], 4,3);
        board.movePiece(board.board[4][3],3,3);
        board.movePiece(board.board[7][3], 4,3);

        String maxColor = "black";
        Player max = board.player1;
        List<Piece> moveablePieces = new ArrayList<>();
        for(Piece piece : max.getPieces()) {
            if(piece.getPieceType().toString().equals("KNIGHT") && piece.calculateLegalMoves(board).size() > 0) {
                moveablePieces.add(piece);
            }
        }
        List<Piece> pieces = max.getPieces();
    ...
    public void testDepth2_1() {
        ...
        MiniMaxAgent a = new MiniMaxAgent(2, false);
        int[] bestMove = {1, 3};
        MoveState idealMove = new MoveState(new Queen(max, 0, 0), bestMove, -1855.0);
        MoveState move = a.minimax(board, moveablePieces, 2, true, maxColor);
    ...
    public void testDepth3() {
        ...
        //The queen will choose the idealmove to capture the king in the next 3 steps
        MiniMaxAgent a = new MiniMaxAgent(3, false);
        int[] bestMove = {2, 5};
        MoveState idealMove = new MoveState(new Queen(max, 0, 0), bestMove, 20265.0);
        MoveState move = a.minimax(board, moveablePieces, 3, true, maxColor);
    ...
    public boolean isEqual(MoveState idealMove, MoveState miniMaxMove)
    {
        ...
        return Arrays.equals(idealMove.getMove(), miniMaxMove.getMove())
                && idealMove.getPiece().getPieceType().equals(miniMaxMove.getPiece().getPieceType());
    }
}